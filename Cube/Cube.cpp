...
#else
#include <unistd.h>  // For usleep function (on non-Windows systems)
#endif
...
int main() {
  ...
  while (1) {
    ...
    #ifdef _WIN32
    Sleep(4); // Sleep for 8 milliseconds (equivalent to 8000 microseconds)
    #else
    usleep(8000); // Sleep to control frame rate (on non-Windows systems)
    #endif
  }
